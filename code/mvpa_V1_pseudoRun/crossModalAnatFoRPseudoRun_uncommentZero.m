function accu = crossModalAnatFoRPseudoRun_uncommentZero(opt)
...
%   opt = chooseMaskNEW(opt);
  ...
  directionNb = [1 2 1 2 2 1 ];
  directionName = {'vertical', 'horizontal','vertical','horizontal','horizontal','vertical' };
  ...
  for iSub = 1:numel(opt.subjects)
    ...
    opt.maskName = {strcat('sub-',num2str(subID),'_hemi-L_space-MNI_label-lV1_radiusNb-',num2str(radiusNb), '.nii'), ...
        strcat('sub-',num2str(subID),'_hemi-R_space-MNI_label-rV1_radiusNb-',num2str(radiusNb), '.nii'), ...
        };
    
%     % group V1 spherical Mask
%     radiusNb=opt.maskRadiusNb;
%     opt.maskPath = fullfile(fileparts(mfilename('fullpath')), '..','..', 'outputs','derivatives' ,'bidspm-roi','groupSphere',strcat('sub-','ALL'));
%     opt.maskName = {strcat('sub-','ALL','_hemi-L_space-MNI_label-lV1_radiusNb-',num2str(radiusNb), '.nii'), ...
%         strcat('sub-','ALL','_hemi-R_space-MNI_label-rV1_radiusNb-',num2str(radiusNb), '.nii')  };
    
    % use in output roi name
    opt.maskLabel = {'lV1', 'rV1'};
