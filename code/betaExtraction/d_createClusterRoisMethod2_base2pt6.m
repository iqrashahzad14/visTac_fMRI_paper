...
for iSub = 1:length(subjectList)
    ...
    opt.inputDir = strcat('/Volumes/IqraMacFmri/visTac/fMRI_analysis/outputs/derivatives/cluster-roi-method2/subjectCluster_base2pt6/',char(subID));
    %fullfile(fileparts(mfilename('fullpath')),'..', '..','outputs','derivatives','cluster-roi-method2','subjectCluster_base2pt6',char(subID));
    inputMaskDir = opt.inputDir;
    
    % where to save the new masks with new reference image
    opt.outputDir = strcat('/Volumes/IqraMacFmri/visTac/fMRI_analysis/outputs/derivatives/cluster-roi-method2/subjectCluster/',char(subID));
    %fullfile(fileparts(mfilename('fullpath')),'..', '..','outputs','derivatives','cluster-roi-method2','subjectCluster',char(subID));
