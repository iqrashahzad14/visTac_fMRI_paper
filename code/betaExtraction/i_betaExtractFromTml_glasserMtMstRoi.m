...
% use each mask - mt, mst from GLASSER ATLAS on tactile motion Localizer
...
for iSub = 1:length(subjectList)
    ...
    opt.maskDir = fullfile(fileparts(mfilename('fullpath')),'..', '..','outputs','derivatives',...
        'bidspm-glasser-roi', 'derivatives', 'bidspm-roi', 'group');
    maskDir = opt.maskDir;
    
    roiName1 = strcat('r','hemi-','L','_space-MNI_atlas-glasser_label-','MST_mask','.nii');
    roiName2 = strcat('r','hemi-','R','_space-MNI_atlas-glasser_label-','MST_mask','.nii');
    roiName3 = strcat('r','hemi-','L','_space-MNI_atlas-glasser_label-','MT_mask','.nii');
    roiName4 = strcat('r','hemi-','R','_space-MNI_atlas-glasser_label-','MT_mask','.nii');
...
xlabel('ROI') 
ylabel('Beta values')
xticklabels({'lMst','rMst','lMT','rMT'})
head1= 'Tactile motion and static beta estimates in Mt, Mst from Glasser '; 
head2='';
title(head1, head2)
