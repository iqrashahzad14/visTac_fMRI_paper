...
donnees <- read.xlsx("betaVal_MtMst_forLmm_onlyMtMst.xlsx")
donnees$subID <- factor(donnees$subID)

donnees <- melt(donnees)
donnees$Cond <- sub("_.*", "", donnees$variable)
donnees$ROI <- sub(".*_", "", donnees$variable)

write.xlsx(donnees, file = "betaVal_MtMst_forLmm_onlyMtMst_longFormat.xlsx")
donnees <- read.xlsx("betaVal_MtMst_forLmm_onlyMtMst_longFormat.xlsx")

donnees$ROI <- factor(donnees$ROI, levels = c("lMst", "lMT", "lhMT","rMst", "rMT", "rhMT"))
ggplot(donnees, aes(x = ROI, y = value, fill = Cond)) + geom_boxplot()

res <- lmer(value ~ ROI*Cond + (1|subID), data = donnees)
...
multComp <- contrast(emmeans(res, ~ROI:Cond), method = "pairwise", simple = "each", combine = TRUE, adjust = "holm")
plot(multComp) + 
  geom_vline(xintercept = 0, linetype = "dotted")

## Pair wise comparison
emmip(res, Cond ~ ROI, CIs = TRUE)
emmip(res, ROI ~ Cond | ROI, CIs = TRUE)
...
#   "fdr", "none")
#lMst, lMT, rMst, rMT
#pVal=c(0.004992266, 0.007901898, 0.000371149,0.003946171 )
