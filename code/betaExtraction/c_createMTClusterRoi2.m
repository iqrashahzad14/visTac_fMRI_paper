...
%this script create rois MT  = part of hMT+ which is not MST by impleenting
%rhMT minus rMst
% MOST UPDATED and CORRECT feb 2024
...
for iSub = 1:length(subjectList)
    ...
    opt.maskDir = fullfile(fileparts(mfilename('fullpath')),'..', '..','outputs','derivatives','cluster-roi-method1','subjectClusterMay2024',char(subID));
    maskDir = opt.maskDir;
...
%             save_nii(roi2,fullfile(opt.outputMask, char(roiNameNewLeft)));
        ...
        elseif strcmp(roiPairLabel,'rMst-rhMT')==1
            ...
            roi4.img = roi4.img - roi3.img;
            roi4.img(find(roi4.img==-1))=0;
...
%             roi4Size(iSub,1) = length(find(roi4.img==1));
            %save new ROIs    
            opt.outputMask = fullfile(fileparts(mfilename('fullpath')),'..', '..','outputs','derivatives','cluster-roi-method1','subjectClusterMay2024',char(subID));
