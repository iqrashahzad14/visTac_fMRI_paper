...
for iSub = 1:numel(opt.subjects)
    ...
    for iRun = 1:numel(dataDir)
        ...
        thisEvent.Properties.VariableNames = {'onset', 'duration', 'trial_type','direction','soundTarget','fixationTarget', 'event', 'block', 'keyName'};
        ...
        for iRow = 1:size(thisEvent,1)-1 %add -1 so that the index does not exceed the last trial
            ...
            % trial_type
            thisTarget = thisEvent{iRow, 'fixationTarget'}; sameTarget = thisEvent{iRow+1, 'fixationTarget'};
            ...
            if thisTarget == 1 && sameTarget == 1
                ...
                accuTacMot=[accuTacMot;thisAccu];
                end
                ...
                accuTacStat=[accuTacStat;thisAccu];
                end
    ...
    accuTacMotSub(iSub,:)=sum(accuTacMot)/length(accuTacMot);
    accuTacStatSub(iSub,:)=sum(accuTacStat)/length(accuTacStat);
end

% save report
% writecell(report,'behaviouralReport.txt');

varNames = ["tacMot","tacStat"];
visAccu = table(accuTacMotSub,accuTacStatSub,'VariableNames', varNames);
writetable(visAccu, "tacLoc.xlsx")
