...
for iSub = 1:numel(opt.subjects)
    ...
    if strcmp(subName, 'sub-pil001')==0
    ...
    for iRun = 1:numel(dataDir)
        ...
        for iRow = 1:size(thisEvent,1)-1 %add -1 so that the index does not exceed the last trial
            ...
            if thisTarget == 1 && sameTarget == 1
                ...
                if strcmp(char(thisTrial),'handDown_pinkyThumb')==1  
                ...
                accuHdPt=[accuHdPt;thisAccu];
                end
                
                if strcmp(char(thisTrial),'handDown_fingerWrist')==1     
                % in the next 5 trials, is there a button press? 
                % 5 trials = 2 stimuli: left +1->left 2resp 3right 4right 5resp
                if iRow+4<=size(thisEvent,1) % the target is the just before the last stimulus, we dont need to go to the iRow+4
                if strcmp(char(thisEvent{iRow+2, 'trial_type'}),'response')==1 ||...
    ...
    elseif strcmp(subName, 'sub-pil001')==1
        ...
        for iRow = 1:size(thisEvent,1)-1 %add -1 so that the index does not exceed the last trial
            ...
            if thisTarget == 1 && sameTarget == 1
                ...
                if strcmp(char(thisEvent{iRow+2, 'trial_type'}),'response')==1 ||...
                        ...
                        strcmp(char(thisEvent{iRow+5, 'trial_type'}),'n/a_n/a') ==1 
                    thisKey = thisEvent{iRow+2, 'keyName'};
                    thisAccu=1;
                else
                    thisKey = 0;
    ...
    accuHdPtSub(iSub,:)=sum(accuHdPt)/length(accuHdPt);
    accuHdFwSub(iSub,:)=sum(accuHdFw)/length(accuHdFw);
end
...
varNames = ["HdPt","HdFw"];
hdAccu = table(accuHdPtSub,accuHdFwSub,'VariableNames', varNames);
% writetable(hdAccu, "handDownAccu.xlsx")
