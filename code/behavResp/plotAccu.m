...
vis =readtable("visAccu.xlsx");
hu =readtable("handUpAccu.xlsx");
hd =readtable("handDownAccu.xlsx");

a=vis.VisVer; b= vis.VisHor;
c=hu.HuPt; d=hu.HuFw;
e=hd.HdPt; f=hd.HdFw;
...
model_error = [
    std(a)/sqrt(length(subList)),std(a)/sqrt(length(subList));...
    std(b)/sqrt(length(subList)),std(b)/sqrt(length(subList));...
    std(c)/sqrt(length(subList)),std(d)/sqrt(length(subList))];

% b = bar(model_series, 'grouped');
bplot = bar(model_series, 'grouped','FaceColor','flat', 'LineWidth',LineWidthMean/2,'BarWidth', 0.9 );

bplot(1).EdgeColor = gray;
bplot(2).EdgeColor = gray;
...
for iiSub=1:length(subList)
    hold on 
    plot([x(1,1), x(2,1)],[a(iiSub), b(iiSub)],'color',darkGray);%,'LineWidth',LineWidthMean/3 )
   
    hold on 
    plot([x(1,2), x(2,2)],[c(iiSub), d(iiSub)],'color',darkGray);%,'LineWidth',LineWidthMean/3)
    hold on 
    plot([x(1,3), x(2,3)],[e(iiSub), f(iiSub)],'color',darkGray);%,'LineWidth',LineWidthMean/3)
...
%         [a(iiSub), b(iiSub),c(iiSub), d(iiSub),e(iiSub), f(iiSub)]);%,'LineWidth',LineWidthMean/3 )
%    
% end
...
xticklabels({'visual','handUp', 'handDown'})
